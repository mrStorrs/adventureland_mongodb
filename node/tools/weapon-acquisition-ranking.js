...
function buildRouteResolver(data, directSourceIndex, shopIndex, medians) {
	...
	const starters = new Set(data.character.starter.weapons || []);
	...
	function resolveAll(itemId) {
		...
		if (starters.has(itemId))
			routes.push({ route_id: `starter:${itemId}`, kind: "starter", source_path: "design/character.js:character.starter.weapons", effort: 0 });
...
function weaponDpsCandidates(data, calculators, weaponId, targetValues) {
	const definition = data.items[weaponId];
	const maximumTarget = Math.max(...targetValues);
	const maximumAttack = Math.max(10000, Math.ceil(Number(definition.attack || 0) * 8), Math.ceil(maximumTarget * 20));
	const candidates = [];
	let lastDps;
	for (let attack = 0; attack <= maximumAttack; attack += 1) {
		const getProperties = (instance) => {
			const properties = calculators.current.calculate_item_properties(instance);
			return instance.name === weaponId && Number(instance.level || 0) === 0 ? { ...properties, attack } : properties;
		};
		const stats = calculateStats({ slots: { mainhand: { name: weaponId, level: 0 } }, items: data.items, getItemProperties: getProperties });
		const dps = stats.attack * stats.frequency;
		if (lastDps === undefined || Math.abs(dps - lastDps) > 1e-12) {
			candidates.push({ attack, dps: roundEvidence(dps), quantum: roundEvidence(stats.frequency) });
			lastDps = dps;
		}
		if (dps > maximumTarget + stats.frequency * 2) break;
	}
	if (!candidates.length) throw new Error(`No representable base attack candidates for ${weaponId}`);
	...
	for (const target of targetValues) {
		...
		while (low < high) {
			...
			if (candidates[middle].dps < target) low = middle + 1;
			else high = middle;
		}
		for (const offset of [-2, -1, 0, 1]) {
			const index = low + offset;
			if (index >= 0 && index < candidates.length) selectedIndexes.add(index);
		}
	}
	return [...selectedIndexes].sort((left, right) => left - right).map((index) => candidates[index]);
}

function compareAssignmentSignatures(left, right) {
	for (let index = 0; index < Math.min(left.length, right.length); index += 1) {
		if (left[index].attack !== right[index].attack) return left[index].attack - right[index].attack;
	...
	function visit(index, selected, cost, minimum, maximum) {
		...
		for (const option of candidateSets.get(row.weapon_id)) {
			visit(
				...
				cost + Math.abs(option.dps - row.assigned_dps_target),
...
function validateAllocatedRows(rows) {
	...
	for (const rank of orderedRanks) {
		...
		if (Math.min(...group.map((row) => row.solved_dps)) + 1e-12 < previousSolvedMaximum)
	...
	const totalError = rows.reduce((sum, row) => sum + Math.abs(row.solved_dps_error), 0);
	const errorBudget = rows.reduce((sum, row) => sum + row.dps_quantum / 2, 0);
	if (totalError > errorBudget + 1e-9) throw new Error(`DPS solver error budget failed for ${rows[0]?.skill}`);
	return { total_error: roundEvidence(totalError), error_budget: roundEvidence(errorBudget) };
}
...
function allocateSkillBudgets(data, calculators, rows) {
	const requirementSlots = rows.map((row) => row.baseline_requirement).sort((left, right) => left - right);
	const dpsTargets = rows.map((row) => row.baseline_dps).sort((left, right) => left - right);
	const ordered = rows.map(clone).sort((left, right) => left.rank - right.rank || left.weapon_id.localeCompare(right.weapon_id));
	for (let index = 0; index < ordered.length; index += 1) {
		ordered[index].assigned_requirement = requirementSlots[index];
		ordered[index].assigned_dps_target = dpsTargets[index];
	}
	const candidateSets = new Map(
		ordered.map((row) => [row.weapon_id, weaponDpsCandidates(data, calculators, row.weapon_id, dpsTargets)]),
	);
	const solution = solveRankedDpsCandidates(ordered, candidateSets);
	const selectedByWeapon = new Map(solution.selections.map((selection) => [selection.weapon_id, selection]));
	for (const row of ordered) {
		const selection = selectedByWeapon.get(row.weapon_id);
	...
	if (!(validation.total_error >= 0)) throw new Error(`Invalid solver result for ${ordered[0]?.skill}`);
	return ordered;
}
...
function buildAcquisitionRanking({ evidence = loadRankingFixture(RANKING_FIXTURE_PATH), data = loadSourceData() } = {}) {
	...
	if (catalog.length !== 80) throw new Error(`Expected 80 legal combat weapons, found ${catalog.length}`);
	...
	const resolver = buildRouteResolver(data, directSources.index, shopIndex, medians);
	...
	for (const skill of COMBAT_SKILLS) {
		const ranked = assignSemanticRanks(
			unallocated.filter((weapon) => weapon.skill === skill),
			Number(evidence.policy.rank_threshold),
		);
		allocated.push(...allocateSkillBudgets(data, calculators, ranked));
	}
	...
	const dpsMultisets = Object.fromEntries(
		COMBAT_SKILLS.map((skill) => [skill, allocated.filter((row) => row.skill === skill).map((row) => row.baseline_dps).sort((left, right) => left - right)]),
	);
...
function markdownReport(fixture) {
	...
	for (const skill of fixture.policy.combat_skills) {
		lines.push(`## ${skill}`, "", "| Rank | Weapon | Effort | Route | Requirement | Target DPS | Solved attack | Solved DPS |", "|---:|---|---:|---|---:|---:|---:|---:|");
		for (const weapon of fixture.weapons.filter((row) => row.skill === skill))
			lines.push(`| ${weapon.rank} | ${weapon.weapon_id} | ${weapon.selected_effort} | ${weapon.selected_route_id} | ${weapon.assigned_requirement} | ${weapon.assigned_dps_target} | ${weapon.solved_attack} | ${weapon.solved_dps} |`);
		lines.push("");
	}
